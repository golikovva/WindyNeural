...
class WindTransformer(nn.Module):
    def __init__(self, in_channels, nhead, num_encoder_layers, batch_first, forecast_range, sequence_len,
                 station_output_channels):
        ...
        self.fea_model.conv1 = new_layer
        self.fea_model = torch.nn.Sequential(*(list(self.fea_model.children())[:-1])).to(device)
        self.transformer = nn.Transformer(nhead=nhead, num_encoder_layers=num_encoder_layers, batch_first=batch_first)
        self.fc = nn.Linear(512, station_output_channels)
    ...
    def forward(self, gfs, station):
        ...
        gfs = self.fea_model(gfs)
        gfs = gfs.view(gfs_batch_size, gfs_seq_len, -1)
        gfs, model_predict = torch.split(gfs, [self.seq_len, self.forecast_range], dim=1)
