...
class WideResNetExtractor(nn.Module):
    def __init__(self, in_channels, depth, widen_factor=1, bn_momentum=0.1, leaky_slope=0.0, dropRate=0.0):
        super(WideResNetExtractor, self).__init__()
        ...
        # default in channels = 3. for sediments we made it 12
        ...
        print(nChannels, 'WideResNet out channels')
...
class build_WideResNet:
    def __init__(self, fea_model_cfg):
        ...
        self.depth = fea_model_cfg.depth
        self.widen_factor = fea_model_cfg.widen_factor
        self.bn_momentum = fea_model_cfg.bn_momentum
        self.dropRate = fea_model_cfg.dropRate
        self.leaky_slope = fea_model_cfg.leaky_slope

    def build(self):
        return WideResNetExtractor(in_channels=self.in_channels,
                                   depth=self.depth,
                                   widen_factor=self.widen_factor,
                                   bn_momentum=self.bn_momentum,
                                   leaky_slope=self.leaky_slope,
                                   dropRate=self.dropRate), 128*self.widen_factor
